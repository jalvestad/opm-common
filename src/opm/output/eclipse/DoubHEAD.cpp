...
{
    // Record 1
    this->data_[Index::TsInit] = tuning.getTSINIT(rptStep);
    this->data_[Index::TsMaxz] = tuning.getTSMAXZ(rptStep);
    this->data_[Index::TsMinz] = tuning.getTSMINZ(rptStep);
    this->data_[Index::TsMchp] = tuning.getTSMCHP(rptStep);
    this->data_[Index::TsFMax] = tuning.getTSFMAX(rptStep);
    this->data_[Index::TsFMin] = tuning.getTSFMIN(rptStep);
    this->data_[Index::TsFcnv] = tuning.getTSFCNV(rptStep);
    this->data_[Index::ThrUPT] = tuning.getTHRUPT(rptStep);
    this->data_[Index::TfDIFF] = tuning.getTFDIFF(rptStep);

    // Record 2
    this->data_[Index::TrgTTE] = tuning.getTRGTTE(rptStep);
    this->data_[Index::TrgCNV] = tuning.getTRGCNV(rptStep);
    this->data_[Index::TrgMBE] = tuning.getTRGMBE(rptStep);
    this->data_[Index::TrgLCV] = tuning.getTRGLCV(rptStep);
    this->data_[Index::XxxTTE] = tuning.getXXXTTE(rptStep);
    this->data_[Index::XxxCNV] = tuning.getXXXCNV(rptStep);
    this->data_[Index::XxxMBE] = tuning.getXXXMBE(rptStep);
    this->data_[Index::XxxLCV] = tuning.getXXXLCV(rptStep);
    this->data_[Index::XxxWFL] = tuning.getXXXWFL(rptStep);
    this->data_[Index::TrgFIP] = tuning.getTRGFIP(rptStep);
    this->data_[Index::TrgSFT] = tuning.getTRGSFT(rptStep);

    // Record 3
    this->data_[Index::TrgDPR] = tuning.getTRGDPR(rptStep);
    this->data_[Index::XxxDPR] = tuning.getXXXDPR(rptStep);
    this->data_[Index::DdpLim] = tuning.getDDPLIM(rptStep);
    this->data_[Index::DdsLim] = tuning.getDDSLIM(rptStep);
